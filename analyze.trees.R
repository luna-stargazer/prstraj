...
source("/home/cmb-00/mde/lindadin/abbeyroad/rhps_coalescent/helper_functions_coal_sel.R")
...
err.array <- array(dim = c(length(time),6,dim(pars)[1]))   
print(dim(err.array))
err.std.array <- array(dim = c(length(time),3,dim(pars)[1]))  
qxtest_mat <- matrix(-1, nrow = dim(pars)[1], ncol = 16)
