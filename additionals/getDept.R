...
departure=data.frame(matrix(nrow=100, ncol=2))
colnames(departure)<-c("RENT+", "Relate")

for(iter in 1:100){


tree_dat <- sprintf("/home/cmb-00/mde/lindadin/abbeyroad/rhps_coalescent/maintext_sims_rent_061318/neutral_sel_%s/out_%s/loci100_sintens0.005_N10000_nchr200_ton%s_toff%s_herit1_%s.RData", ts, iter, ts, t.offs, iter)
...
departure[iter,]=c(length(which(trajs_neut_rent[4001,]!=0))/100, length(which(trajs_neut_relate[4001,]!=0))/100)
...
save.image(paste("/home/cmb-00/mde/lindadin/abbeyroad/rhps_coalescent/maintext_sims_rent_061318/departure", ".RData", sep = ""))
