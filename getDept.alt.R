...
departure.alt=data.frame(matrix(nrow=100, ncol=2))
colnames(departure.alt)<-c("RENT+", "Relate")
...
departure.alt[iter,]=c(length(which(trajs_neut_rent[4001,]!=0))/100, length(which(trajs_neut_relate[4001,]!=0))/100)



}
assign(sprintf("departure.alt%s", ts), departure.alt)

}


save.image(paste("/home/cmb-00/mde/lindadin/abbeyroad/rhps_coalescent/maintext_sims_rent_061318/departure.alt", ".RData", sep = ""))
